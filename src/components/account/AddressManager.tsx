...
export function AddressManager() {
  ...
  const [formData, setFormData] = useState<AddressRequestDTO>({
    ...
    neighborhood: "",
    city: "",
    state: "SP", // default value or cast it when sending
    postalCode: "",
    isDefault: false,
  ...
  const fetchAddresses = async () => {
    ...
    try {
      ...
      const sorted = data.sort((a, b) => (b.isDefault === a.isDefault ? 0 : b.isDefault ? 1 : -1));
      setAddresses(sorted);
    } catch (error) {
  ...
  const handleCreateAddress = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await fetchAddresses(); // Refresh list to get correct defaults
      setIsFormOpen(false);
      setFormData({
  ...
  if (isFormOpen) {
    return (
      <div className="border border-muted p-6 sm:p-8 bg-background max-w-2xl animate-in fade-in">
        ...
        <form onSubmit={handleCreateAddress} className="flex flex-col gap-5">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
            <div className="flex flex-col gap-1 sm:col-span-2">
              <label className="text-xs text-muted-foreground uppercase tracking-widest">CEP</label>
              <input type="text" required value={formData.postalCode} onChange={(e) => setFormData({ ...formData, postalCode: e.target.value })} className="w-full bg-transparent border-b border-muted py-2 text-sm focus:outline-none focus:border-foreground transition-colors" />
            </div>
